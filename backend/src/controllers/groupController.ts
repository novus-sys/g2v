...
export const createGroup = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    ...
    return res.status(500).json({ message: 'Error creating group', error: 'Unknown error occurred' });
  }
};
...
export const getGroups = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    return res.status(500).json({ message: 'Error fetching groups', error: 'Unknown error occurred' });
  }
};
...
export const getGroup = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    return res.status(500).json({ message: 'Error fetching group', error: 'Unknown error occurred' });
  }
};
...
export const updateGroup = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    ...
    return res.status(500).json({ message: 'Error updating group', error: 'Unknown error occurred' });
  }
};
...
export const joinGroup = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    ...
    return res.status(500).json({ message: 'Error joining group', error: 'Unknown error occurred' });
  }
};
...
export const leaveGroup = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    ...
    if (error instanceof Error) {
      return res.status(500).json({ message: 'Error leaving group', error: error.message });
    }
    return res.status(500).json({ message: 'Error leaving group', error: 'Unknown error occurred' });
  }
};

// Delete group
export const deleteGroup = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
      return res.status(400).json({ message: 'Invalid group ID format' });
    }
    ...
    if (group.status === 'completed') {
      return res.status(400).json({ message: 'Cannot delete a completed group' });
    }
    ...
    return res.status(500).json({ message: 'Error deleting group', error: 'Unknown error occurred' });
  }
};

// Kick member from group
export const kickMember = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
      return res.status(400).json({ message: 'Invalid group ID format' });
    }
    ...
    if (group.creator.toString() !== req.user._id) {
      return res.status(403).json({ message: 'Not authorized to kick members from this group' });
    }
    ...
    if (error instanceof Error) {
      return res.status(500).json({ message: 'Error kicking member', error: error.message });
    }
    return res.status(500).json({ message: 'Error kicking member', error: 'Unknown error occurred' });
  }
};

// Transfer group ownership
export const transferOwnership = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
      return res.status(400).json({ message: 'Invalid group ID format' });
    }
    ...
    if (group.creator.toString() !== req.user._id) {
      return res.status(403).json({ message: 'Not authorized to transfer ownership of this group' });
    }
    ...
    return res.status(500).json({ message: 'Error transferring ownership', error: 'Unknown error occurred' });
  }
};

// Update group status
export const updateStatus = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
      return res.status(400).json({ message: 'Invalid group ID format' });
    }
    ...
    if (group.creator.toString() !== req.user._id) {
      return res.status(403).json({ message: 'Not authorized to update status of this group' });
    }

    // Validate status transitions
    if (group.status === 'completed' && status !== 'completed') {
      return res.status(400).json({ message: 'Cannot change status of a completed group' });
    }

    if (status === 'open' && group.members.length >= group.maxMembers) {
      return res.status(400).json({ message: 'Cannot set status to open when group is full' });
    }

    if (status === 'open' && new Date(group.expiryDate) <= new Date()) {
      return res.status(400).json({ message: 'Cannot set status to open when group has expired' });
    }
    ...
    if (error instanceof Error) {
      return res.status(500).json({ message: 'Error updating group status', error: error.message });
    }
    return res.status(500).json({ message: 'Error updating group status', error: 'Unknown error occurred' });
  }
}; 